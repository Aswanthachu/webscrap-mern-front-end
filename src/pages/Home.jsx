...
const Home = () => {
    ...
    const scrapingProcess = async (webLink) => {
      ...
      try {
        const scrapedData = await axios.post(
          "http://webscrap-mern-backend.vercel.app/text/text-scrap",
          { web: webLink }
        );
  ...
  useEffect(() => {
    async function scraping() {
      const scrap = await axios.get(
        "http://webscrap-mern-backend.vercel.app/text/get-all-scraped-details"
      );
      if (scrap) {
  ...
  const handleDelete = async (e, id) => {
    ...
    try {
      await axios.delete(
        `http://webscrap-mern-backend.vercel.app/text/remove-one-scraped-data/${id}`
      );
  ...
  const handleAddFav = async (e, id) => {
    ...
    console.log(id);
    try {
      await axios.patch(`http:/webscrap-mern-backend.vercel.app/text/add-to-favourite/${id}`);
